...
import { Home, ArrowLeft, SearchX, Globe, Compass, FileCode2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between p-6 md:p-12 font-sans select-none">
      ...
      <main className="max-w-6xl mx-auto w-full py-12 my-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
          ...
          {/* Bloc 1: Le grand message & actions */}
          <div className="md:col-span-7 rounded-3xl bg-muted/30 border border-border/60 p-8 md:p-12 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="space-y-4 relative z-10">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* Badge affichant la route problématique */}
                {problematicRoute && (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-destructive/20 bg-destructive/10 px-3 py-1 font-mono text-xs font-medium text-destructive max-w-xs truncate">
                    <FileCode2 className="h-3.5 w-3.5 shrink-0" />
                    <span className="truncate">{problematicRoute}</span>
                  </span>
                )}
              </div>
              ...
              <p className="text-muted-foreground text-base md:text-lg leading-relaxed max-w-md">
                L'adresse demandée ne pointe vers aucun service ou module actif de l'écosystème <strong className="text-foreground">{SITE.name}</strong>.
              </p>
            </div>
            ...
            <div className="pt-8 flex flex-wrap gap-4 relative z-10">
              ...
              <button
                ...
                className="inline-flex items-center gap-2.5 rounded-2xl border border-border bg-card px-6 py-3.5 font-sans text-sm font-semibold text-foreground transition-all hover:bg-muted active:scale-95 shadow-xs"
              >
          ...
          {/* Bloc 2: Statut de la requête & détails réseau */}
          <div className="md:col-span-5 flex flex-col gap-6">
            <div className="flex-1 rounded-3xl border border-border/60 bg-card p-6 md:p-8 flex flex-col justify-between shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-muted-foreground">INSPECTION DE LA ROUTE</span>
                <Globe className="h-4 w-4 text-muted-foreground" />
              </div>

              <div className="my-6 space-y-3 font-mono text-xs">
                {/* Ligne spécifique pour la Route Manquante */}
                <div className="flex items-center justify-between border-b border-border/40 pb-2 gap-2">
                  <span className="text-muted-foreground shrink-0">Route ciblée</span>
                  <span className="font-semibold text-destructive truncate max-w-50" title={problematicRoute}>
                    {problematicRoute || "Ruta non résolue"}
                  </span>
                </div>

                <div className="flex justify-between border-b border-border/40 pb-2">
                  <span className="text-muted-foreground">Statut HTTP</span>
                  <span className="font-bold text-destructive">404 (Not Found)</span>
                </div>

                <div className="flex justify-between border-b border-border/40 pb-2">
                  <span className="text-muted-foreground">Hôte actuel</span>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Vérifiez l'orthographe du chemin <code className="bg-muted px-1.5 py-0.5 rounded text-foreground font-mono">{problematicRoute || "/..."}</code> ou utilisez la navigation officielle.
              </p>
            </div>
          </div>
